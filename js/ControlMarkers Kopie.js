L.Control.SimpleMarkers = L.Control.extend({
    ...
    onMapClickAddMarker: function(e) {
        ...
        // Variablen
        var c, KP, KW, AlphaW, AlphaZ, AlphaKP, AlphaB, AlphaC, BetaP, BetaL, Gamma, KursWinkel, KursLinienlaenge,
          ShipPoint_analog, ShipPoint_dezimal, WayPoint_analog, WayPoint_dezimal, WindStaerke, schnellsterWinkel;

        // Schiffsposition in analog "48°29′26″N, 011°20′18″E" und dezimal LatLng(48.49054, 11.33821)
        ShipPoint_analog = new LatLon(Geo.parseDMS(myLat), Geo.parseDMS(myLng));
        ShipPoint_dezimal = new L.LatLng(myLat, myLng);

        // Wegpunkt
        WayPoint_analog = new LatLon(Geo.parseDMS(e.latlng.lat), Geo.parseDMS(e.latlng.lng));
        WayPoint_dezimal = new L.LatLng(e.latlng.lat, e.latlng.lng);
                                           
				// Kurs "KP" (Bsp: 338.12305164292053 in Grad) und Distanz "c" (Bsp: 1.2345 in km) vom Schiff zum Wegpunkt
				KP = ShipPoint_analog.bearingTo(WayPoint_analog);  // Kurs "KP" zum Wegpunkt
				c = ShipPoint_analog.distanceTo(WayPoint_analog);  // Distanz "c" nach vergrößerter Breite, in km
		
        // Optionen: KW = Windrichtung - aus Page: windPage / Input des Drehknopfes entnehmen (Bsp: 275 in Grad)
        KW = parseFloat($('#windRichtung').val());
                                           
        schnellsterWinkel = 50;                           // Berechnet aus Spline-Interpolation

				// BeatVMG_Angel für [6..20]kn Wind - aus Scroller der Wind-Page und VPP-Tabelle entsprechend der Windstärke
        eval("BetaL = parseFloat(ship_" + $('#windSpeed').val() + "_beat_angel);");
        ...
        if (AlphaC > AlphaB) { Gamma = 180 - AlphaB - BetaP; }
        else { Gamma = 180 - AlphaC - BetaP; }
        ...
        // Berechnung der Kurslinie - erster Schlag zum Wegpunkt
        ...
        Kurs = new L.polyline(BearingCoordinates, {
          color: 'blue', weight: 2, opacity: 0.5, smoothFactor: 1
        }).addTo(map);
                                           
        // Darstellung Koordinaten, Kurs und Distanz
        //var popupContent = "φ: " + dd2dm(e.latlng.lat, 'lat', 5) + "<br/>λ: " + dd2dm(e.latlng.lng, 'lng', 5) + "<br/>Distanz: " + km2nm(c) + "nm<br/>Peilung: " + round(KP,2) + "\u00b0<br/>AlphaC: " + round(AlphaC,2) + "/AlphaB: " + round(AlphaB,2) + "<br/>AlpahKP: " + round(AlphaKP,2) + "/BetaP: " + round(BetaP,2) + "<br/>KW aus: " + round(KW,2) + "/KW in: " + round(KWrichtung,2) + "<br/>Kurs: " + round(KursWinkel,2);
        ...
        // Direkte Peilunglinie vom Wegpunkt zum Schiff
